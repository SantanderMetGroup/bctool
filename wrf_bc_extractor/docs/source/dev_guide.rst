...
First activate the ``wrf_bc_extractor`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n wrf_bc_extractor --explicit > spec-file.txt
...
Make a new version of wrf-bc-extractor in the following steps:
